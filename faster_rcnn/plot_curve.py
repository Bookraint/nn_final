...
def plot_loss_and_lr(train_loss, learning_rate, model_name):
    try:
        ...
        fig.savefig('./{}loss_and_lr.png'.format(model_name), dpi=300)
        plt.close()
...
def plot_map(mAP, model_name):
    try:
        ...
        plt.xlim(0, len(mAP))
        plt.legend(loc='best')
        plt.savefig('./{}_mAP.png'.format(model_name), dpi=300)
        plt.close()
